...
- **Webhook mode (instant review, `REVIEW_WEBHOOK` default on):** enable the webhook in a repo's edit dialog and it issues a per-repo URL + secret. Paste those into the host's webhook settings and a new PR or a fresh push starts its review **immediately**, without waiting for the interval. Authentication follows each provider: HMAC signature for GitHub (`X-Hub-Signature-256`), the secret-token header for GitLab, `?token=` in the URL for Bitbucket (which has no secret field). Only PR events are acted on — comment/push noise is ignored — and with webhooks in place interval polling can be turned off entirely (`REVIEW_POLL_MS=0`)
