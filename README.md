...
- **Pull, commit & push** a cloned repo from the chat header — or from My Page › Projects, where every personal project has its own Git button, so a project does not have to be attached to a chat to be pulled (or let Claude do it) with encrypted per-user git credentials. Pull is fast-forward only by default; tick *Pull with rebase* to replay diverged local commits on top instead
