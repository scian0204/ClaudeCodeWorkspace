...
### 🤝 Shared plans · 📦 Per-session build container · 🪟 Windows builds · 🌐 Browser
...
**A browser for Claude** — turn the **Browser** pill on (or type `/browser`) and that chat gets a headless browser: Claude can open web pages, click and type, read the page as an accessibility tree, take screenshots — which appear in the chat as images — and read the page's console and network requests. It reaches the chat's own dev server too, whether that runs in the app container or in the chat's build container; the one rule is that the server must listen on `0.0.0.0` (vite: `--host 0.0.0.0`), because the browser runs in its own container and `localhost` there is the browser itself. One Playwright container serves the whole workspace: every chat's turn gets a private browser context inside it (its own tabs, cookies and storage — two chats never see each other's pages), and the container is removed once nobody has used it for a while, which also cleans up anything a turn left open. Off by default; an admin turns it on (`browserEnabled`, `browserImage`, `browserMemMB`, `browserIdleMs`). Not used for PR review.
...
| 🌐 | **A browser for Claude** | Turn the **Browser** pill on and the chat can open web pages, click and type, read the page as text, take screenshots (shown in the chat as images) and read the page's console and network — including its own dev server running in the app or build container. One headless-browser container for the whole workspace, a private browser context per chat, removed after idling (`browserEnabled`). |
