...
- **Import a local session** — upload a project folder plus its `~/.claude` session files to clone the conversation as a resumable private session. Transcripts the CLI never named show a snippet of their first message instead of a raw uuid, and you can tick a box on the import screen to have their conversation read and named properly (`importAutoTitleEnabled`). Anything you already imported is flagged in the picker, with a per-session choice to overwrite it or add a copy
