...
- **Team agents** — define custom agents (name, description, system prompt, allowed tools, model) once, admin-managed for the whole team or personal per member; every session gets them as Task-tool subagents, and a chat-header pill can put one in charge of the **main thread** from the next turn onward (`teamAgentsEnabled`)
