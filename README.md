...
- **See what runs behind the answer** — a *Tasks* pill in the chat header opens a panel beside the conversation listing every subagent, backgrounded shell, workflow and MCP monitor the turn spawned: live status, elapsed time, token + tool-call counts, the tool each one is on right now, and its final summary or error (filterable by agents · shells · workflows). Tool calls a subagent made are badged as such in the transcript instead of reading as main-thread calls; on a phone the panel opens full-screen. Admins can turn it off (`taskPanelEnabled`). A **Live** toggle on any running subagent opens a tmux-style pane inside the panel — that agent's own tool calls and streaming text, pinned to the bottom as it works — and a **split view** button opens every agent's pane at once. The CLI's experimental **agent teams** (`CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS`) is enabled per session (`agentTeamsEnabled`): teammates run as named background agents and stream into the same panes; concurrent teams currently need a bypass-mode session (an upstream SDK bug keeps background agents foreground when approval prompts are possible)
- **Live usage meter** in the chat header — per-session **context-window** fill plus your **claude.ai plan limits** (5-hour, weekly, per-model) with reset countdowns, straight from the CLI. A pasted `claude setup-token` is inference-only (no `user:profile` scope), so when you also have a browser sign-in the limit lookup automatically uses that full-scope credential instead — and when you don't, the popover tells you to sign in from My Page rather than blaming your plan. API-key / Bedrock / custom providers have no plan window at all (per-token billing)
