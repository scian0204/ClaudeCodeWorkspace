...
- **Import a local session** — upload a project folder plus its `~/.claude` session files to clone the conversation as a resumable private session. Transcripts the CLI never named are titled from their own conversation instead of showing a raw uuid (`importAutoTitleEnabled`)
