...
- **워크스페이스 통합 검색**(`Ctrl/Cmd+K`) — 개인 대화, 참여 중인 공통 세션(방), DM·그룹 메시지, 프로젝트, LLM Wiki 주제·문서, PR 코드리뷰, 사람까지 한 번에 검색. 결과를 누르면 해당 메시지로 바로 이동(문서는 파일 탐색기로 열림). 기본은 **기능별 그룹**, 필요하면 **최신순 / 오래된순** 통합 정렬로 전환. 각 항목은 **원래 볼 수 있는 범위만** 검색됨
