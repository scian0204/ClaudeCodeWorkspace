...
- **전자동 파이프라인**(대화 불필요, `REVIEW_AUTO` 기본 on): 새 PR마다 서버가 **로컬 머지**(공유 클론에서 파생한 PR별 워크트리에 `--no-ff`; 충돌이면 중단·표시)한 뒤, **무인 에이전트 턴**이 **빌드·실행**, **버그 감지**, **diff 코드 리뷰**를 하고 **`VERDICT: MERGE_SAFE` / `DO_NOT_MERGE`** + 한 줄 요약을 낸다. 판단은 세션·사이드바 배지에 표시, 언제든 재실행
...
> **자동 리뷰는 PR 코드를 실행합니다.** 자동 PR 리뷰 파이프라인은 각 PR의 빌드/실행 스크립트를 무인으로 돌립니다(도구 자동 승인). 병합권한 git 자격증명은 해당 턴에서 제외하지만, 그 턴의 Claude 토큰은 프로세스 env에 남아 있으므로 자동 리뷰는 **신뢰된 코드 실행**으로 취급하세요. 외부의 신뢰할 수 없는 PR을 받는 저장소라면 `REVIEW_AUTO=0`(사람이 직접 트리거)로 두거나 감시하지 마세요. 빌드 샌드박스가 업그레이드 경로입니다.
