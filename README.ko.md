...
- 클론한 저장소를 채팅 헤더에서 **가져오기(pull) · 커밋 · 푸시** (Claude가 대신 해줄 수도) — 암호화된 유저별 git 자격증명 사용. 마이페이지 › 프로젝트 목록에서도 프로젝트별 Git 버튼으로 같은 패널을 열 수 있어, 채팅에 붙이지 않은 프로젝트도 바로 pull 가능. pull 은 기본 fast-forward 전용이고, 로컬 커밋이 갈라졌으면 *리베이스로 가져오기* 를 켜면 됩니다
