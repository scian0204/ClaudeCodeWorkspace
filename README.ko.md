...
- **전용 우클릭 메뉴** — 우클릭하면 브라우저 기본 메뉴 대신 워크스페이스 동작이 뜸: 새 대화·검색·사이드바·테마·새로고침, 입력창에서는 복사/잘라내기/붙여넣기, 항목별로는 대화 이름 변경·삭제, 대화방·위키 주제 삭제, 메시지 복사·수정·삭제. `Shift`+우클릭은 브라우저 기본 메뉴를 그대로 냄. 관리자 설정(`customContextMenu`)으로 기능 전체를 끌 수 있음
