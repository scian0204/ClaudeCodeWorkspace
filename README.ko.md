...
- **로컬 세션 가져오기** — 프로젝트 폴더와 `~/.claude` 세션 파일을 업로드해 대화를 resume 가능한 개인 세션으로 복제. CLI에서 이름을 붙이지 않은 세션은 uuid 대신 첫 메시지로 이름이 붙고, 가져오기 화면에서 체크하면 대화내역을 읽어 제대로 된 이름을 지어 줌(`importAutoTitleEnabled`)
