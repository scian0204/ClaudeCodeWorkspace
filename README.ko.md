...
- 문서를 올리면 Claude가 컴파일해 주는 **LLM Wiki** 팀 지식 기반 — 기존 대화나 프로젝트 파일에서 시작하거나 빈 위키로 시작할 수도 있고, 어떤 대화·대화방에든 주제를 **연결**해 그 지식을 근거로 답하게 할 수 있다. 주제를 **대화로 지식 쌓기**로 켜두면 오간 대화에서 남길 만한 내용을 Claude가 스스로 판단해 물어보고 넣거나 바로 넣는다. 파일 탐색기에서는 완성된 지식 기반을 **관계 그래프**로 볼 수 있다 — 아티클 하나가 점 하나, 아티클끼리 걸린 링크가 선 하나이고, 점을 누르면 그 문서가 바로 열린다(`wikiGraphMaxNodes`)
- **로컬 세션 가져오기** — 프로젝트 폴더와 `~/.claude` 세션 파일을 업로드해 대화를 이어서 쓸 수 있는 개인 세션으로 복제. CLI에서 이름을 붙이지 않은 세션은 긴 임의 id 대신 첫 메시지로 이름이 붙고, 가져오기 화면에서 체크하면 대화내역을 읽어 제대로 된 이름을 지어 줌(`importAutoTitleEnabled`). 폴더는 그대로 두고 **세션만** 가져올 수도 있음 — 첫 화면에서 이미 워크스페이스에 있는 프로젝트(내 것 또는 공용)를 고르면 업로드 없이 대화내역만 들어옴. 이미 있는 프로젝트와 세션은 각각 중복 표시되며 덮어쓰기/복제를 고를 수 있고, 프로젝트 폴더를 덮어쓸 땐 기존 파일을 유지할지 지울지도 함께 선택
...
- **5시간 창 자동 선점** (Claude 구독 전용) — claude.ai 5시간 창은 시계가 아니라 **첫 질의 시점**부터 카운트되므로, 리셋 후 그냥 두면 그 시간이 그대로 날아감. 켜 두면 5시간 카운트가 돌고 있지 않을 때 서버가 아주 짧은 질문(저렴한 모델, 대화 목록에는 남지 않음)을 한 번 대신 보내 카운트를 시작시킴 — 실제로 앉아서 일할 때 5시간이 온전히 남아 있게 하려는 것. 마이페이지에서 사용자별로 켜고(마지막으로 보낸 시각 표시), 관리자 설정(`windowPrimer*`)으로 조정하거나 끌 수 있음. 하루 종일 도는 게 싫으면 같은 자리에서 **시간을 지정**할 수 있음 — 허용 시간대(예: 09:00~19:00)와 지정 시각(예: 09:00, 14:00)을 각각 또는 함께 설정하며, 브라우저의 시간대를 기준으로 동작함
...
### 🤝 요금제 함께 쓰기 · 📦 세션 전용 빌드 컨테이너 · 🪟 Windows 빌드 · 🌐 브라우저
...
**Claude가 쓰는 브라우저** — 헤더의 **브라우저** 버튼을 켜면(또는 `/browser`) 그 대화에 화면 없이 실행되는 브라우저가 붙습니다. Claude가 웹 페이지를 열고, 클릭·입력하고, 페이지를 글(접근성 트리)로 읽고, 스크린샷을 찍고 — 스크린샷은 대화에 이미지로 나옵니다 — 페이지의 콘솔 메시지와 네트워크 요청도 봅니다. 앱 컨테이너든 그 대화의 빌드 컨테이너든, 자기 개발 서버에도 그대로 접속합니다. 규칙은 하나, 개발 서버가 `0.0.0.0` 에 떠 있어야 합니다(vite는 `--host 0.0.0.0`) — 브라우저가 별도 컨테이너에서 돌기 때문에 거기서 `localhost` 는 브라우저 자기 자신이라서요. Playwright 컨테이너 하나가 워크스페이스 전체를 맡되, 대화의 각 질문은 그 안에서 자기만의 브라우저 컨텍스트(탭·쿠키·저장소 분리)를 받아 두 대화가 서로의 페이지를 볼 일이 없고, 한동안 아무도 쓰지 않으면 컨테이너를 내리면서 끝내지 않고 남겨진 창도 함께 정리합니다. 기본은 꺼짐이고 관리자가 켭니다(`browserEnabled`, `browserImage`, `browserMemMB`, `browserIdleMs`). PR 리뷰에는 쓰지 않습니다.
...
<tr>
<td valign="top"><b>3단 플러그인</b> — 공통(관리자)·프로젝트(그 프로젝트를 쓰는 모두)·개인(유저) 티어; git·마켓·업로드 설치, 플러그인별 상세 + 원클릭 업데이트.</td>
...
| 🌐 | **Claude가 쓰는 브라우저** | 헤더의 **브라우저** 버튼을 켜면 그 대화가 웹 페이지를 열고, 클릭·입력하고, 페이지를 글로 읽고, 스크린샷을 찍고(대화에 이미지로 표시), 콘솔·네트워크를 확인할 수 있습니다 — 앱 컨테이너나 빌드 컨테이너에서 돌고 있는 자기 개발 서버도 포함. 화면 없이 실행되는 브라우저 컨테이너 하나를 워크스페이스 전체가 나눠 쓰되 대화마다 탭·쿠키는 따로이고, 한동안 안 쓰면 내려갑니다(`browserEnabled`) |
...
| 🔌 | **플러그인: 공통 · 프로젝트 · 개인** | 공통(관리자)·개인(유저) 티어에 **프로젝트** 티어 추가 — 프로젝트에 설치한 플러그인은 그 프로젝트를 쓰는 모든 대화가(누구 대화든) 함께 불러오므로, 저장소 전용 도구를 각자 다시 설치할 필요가 없음. git·마켓플레이스·로컬 업로드 설치, 관리자 필수강제, 유저별 on/off. 플러그인별 상세 보기 + 원클릭 업데이트 |
| 🏢 | **회사 계정 로그인: AD/LDAP · SSO** | 이미 쓰는 회사 계정으로 들어올 수 있는 두 가지 경로. **AD/LDAP**: 평소 로그인 창에 넣은 사내 아이디·비밀번호를 회사 디렉터리에 그대로 확인시킨다(조회 계정으로 검색한 뒤 그 사람의 DN으로 다시 바인드, StartTLS/LDAPS 지원). 비밀번호는 이쪽에 저장하지 않는다. **SSO(OpenID Connect)**: 로그인 창에 버튼이 하나 늘고, 실제 인증은 IdP(Entra ID·Keycloak·Okta 등)가 한다 — Authorization Code + PKCE, id_token은 IdP 서명 키로 검증. 둘 다 처음 로그인한 사람의 계정을 자동 생성할 수 있고, AD 그룹이나 SSO 그룹 claim으로 관리자 여부를 정할 수도 있다. 관리자는 디렉터리 사용자를 한 번에 가져오거나 주기적으로 다시 가져오게 할 수 있다. 이미 있는 로컬 계정을 같은 아이디로 넘겨받는 동작은 기본 꺼짐 — AD에 `admin`을 만들 수 있는 사람이 이 워크스페이스 관리자 계정을 가져가지 못하게. 접속 정보는 암호화 저장되며 관리자 패널 **로그인 연동** 탭에서 설정. 플래그는 `ldapEnabled`·`oidcEnabled`, 그리고 `localLoginEnabled`(끄면 멤버는 아이디/비밀번호 창을 못 쓰지만 관리자는 계속 가능 — 디렉터리가 죽어도 잠기지 않게) |
...
| 💭 | **사이드 채팅 (`/btw`)** | 진행 중인 작업에 대해, 그 대화에 남기지 않고 물어보기. 대화 위에 작은 창이 뜨고 대화를 *복사한* 상태에서 답하므로 Claude는 지금까지 오간 내용을 다 알지만, 대화 기록은 그대로 남고 다음 실제 턴도 영향을 받지 않음. 읽기 전용(파일 조회는 되지만 수정·명령 실행은 불가)이며 저장하지 않음 — 물어본 탭에서만 살아 있음. 메시지 입력창 아래 **/btw** 버튼으로 열 수 있고, `/btw` 를 입력해도 됨(`/btw 질문` 이면 바로 물어봄). 관리자 플래그 `asideEnabled` |
...
- 프로젝트 티어 = 프로젝트에 설치하면 그 프로젝트를 쓰는 모든 대화가(누구 대화든) 함께 불러옴. 공통 프로젝트는 관리자, 개인 프로젝트는 주인이 관리. 플러그인은 프로젝트 폴더 **밖에** 저장되므로 저장소에 섞이지 않고, 프로젝트를 지우면 같이 지워짐
- 개인 티어 = 유저 자유(마켓 추가·설치·공통 클래스2 on/off)
- git 주소를 받는 곳이면 어디든 `foo/bar`로 짧게 — 마켓 추가는 그 한 칸이면 되고, 이름은 마켓이 자기 매니페스트에 적어둔 값을 쓴다
- 등록한 마켓플레이스에서 설치: 마켓 줄을 열면 목록이 나오고 버튼 하나로 설치, 또는 `plugin@market` 입력. **업데이트**로 마켓 저장소를 다시 받아 새로 올라간 플러그인 확인
- 플러그인 이름으로 설치: 이름 칸에 등록한 마켓의 플러그인 이름(같은 이름이 둘이면 `plugin@market`), git 칸은 저장소에서 바로 설치할 때만
...
| `WIN_DOCKER_HOST` | .NET Framework 빌드를 돌릴 Windows 도커 주소, 예: `tcp://win-build:2376` (비우면 기능 꺼짐) | — |
| `WIN_DOCKER_CERT_DIR` | 그 호스트용 `ca.pem`/`cert.pem`/`key.pem` 폴더. 값이 있으면 TLS. 컨테이너에 마운트하세요 | — |
...
- [x] 회사 계정 로그인: AD/LDAP + OIDC SSO (첫 로그인 시 계정 생성, 디렉터리 가져오기, 그룹→역할)
- [ ] 프록시 헤더 인증 어댑터 · SAML
