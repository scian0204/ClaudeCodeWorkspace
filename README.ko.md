...
- **풀리퀘스트 리뷰** — 관리자가 저장소를 감시, 열린 PR마다 세션이 생겨 로컬 머지·빌드/실행·리뷰
...
| 🔀 | **PR 리뷰 세션** | 관리자가 원격지를 등록(병합권한 자격증명 필요), 서버가 GitHub/GitLab/Bitbucket을 폴링해 열린 PR마다 리뷰 세션 생성 — 관리자와 PR 작성자(읽기 전용)만 열람. 한 번 클릭으로 PR을 PR별 git 워크트리에 로컬 머지하고, 거기서 Claude가 빌드·실행·디버깅(요청 시)·리뷰 |
...
- **열람 권한:** 관리자는 전체, PR 작성자(로컬 계정과 username으로 매칭)는 자기 것만 **읽기 전용**(스트림 관전만, 전송/머지 불가). 매칭 계정 없으면 추가 열람자 없음
- **로컬 머지:** 클릭 한 번으로 PR head를 받아 공유 클론에서 파생한 PR별 git 워크트리에 `--no-ff` 머지, 충돌은 트리에 남겨 해결. 원격에는 아무것도 푸시하지 않음
- 머지 후에는 대화만: Claude에게 빌드·실행·디버깅·리뷰를 요청하면 머지된 워크트리에서 수행
</details>
