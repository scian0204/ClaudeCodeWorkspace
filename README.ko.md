...
- **답변 뒤에서 돌아가는 것들 보기** — 채팅 헤더의 *작업* pill을 누르면 대화 오른쪽에 패널이 열려, 그 턴이 뒤에서 돌린 **서브에이전트·백그라운드 셸·워크플로·MCP 모니터**가 전부 나옴: 실행 상태, 경과 시간, 토큰·도구 호출 수, 지금 쓰고 있는 도구, 끝난 뒤의 요약/오류(에이전트 · 셸 · 워크플로 탭으로 필터). 서브에이전트가 실행한 도구 카드는 대화 본문에서도 배지로 구분되어 메인 스레드 호출과 헷갈리지 않고, 폰에서는 패널이 전체화면으로 열림. 관리자 설정(`taskPanelEnabled`)으로 끌 수 있음. 실행 중인 서브에이전트 행의 **실시간** 토글을 켜면 패널 안에 tmux처럼 그 에이전트 전용 창이 열려, 도구 호출과 스트리밍 텍스트를 바닥에 고정된 채로 같이 볼 수 있고, **분할 보기** 버튼으로 모든 에이전트의 창을 한 번에 펼침. CLI의 실험적 **에이전트 팀**(`CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS`)도 세션마다 켜짐(`agentTeamsEnabled`) — 팀원은 이름 붙은 백그라운드 에이전트로 돌며 같은 창으로 스트리밍됨. 동시 실행 팀은 당분간 bypass 모드 세션에서만(승인 프롬프트가 가능한 모드에선 업스트림 SDK 버그 때문에 백그라운드 에이전트를 포그라운드로 고정)
