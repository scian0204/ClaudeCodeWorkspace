...
- **웹훅 방식(즉시 리뷰, `REVIEW_WEBHOOK` 기본 on):** 저장소 편집 창에서 웹훅을 켜면 저장소별 URL + 시크릿이 발급된다. 이를 호스트의 웹훅 설정에 넣으면 PR이 열리거나 새 커밋이 푸시될 때 폴링 주기를 기다리지 않고 **즉시** 조회·리뷰가 시작된다. 인증은 GitHub는 HMAC 서명(`X-Hub-Signature-256`), GitLab은 시크릿 토큰 헤더, 시크릿 필드가 없는 Bitbucket은 URL의 `?token=`. PR 이벤트만 처리하고 코멘트·푸시 등 잡음은 무시하며, 웹훅을 걸었다면 `REVIEW_POLL_MS=0`으로 폴링을 아예 끌 수 있다
