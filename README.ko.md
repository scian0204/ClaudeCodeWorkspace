...
- **변경 내용(diff) 보기 · 히스토리 그래프** — Git 패널에서 변경 파일 이름을 누르면 그 파일의 uncommitted patch(추적되지 않은 새 파일도 전체가 추가된 형태로), 히스토리 섹션을 펼치면 브랜치·머지가 색깔 레인으로 그려진 커밋 그래프. 커밋을 누르면 그 커밋의 stat + patch (*모든 브랜치* 토글로 HEAD 밖의 브랜치까지, 상한은 `gitLogMaxCount`/`gitDiffMaxKB`)
