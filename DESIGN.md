...
- **통합 검색:** `Ctrl/Cmd+K` 팔레트 하나로 워크스페이스 전체(개인 대화·참여 중인 방·DM/그룹 메시지·프로젝트·LLM Wiki 주제/문서·PR 리뷰·사람) 검색. 결과 클릭 시 해당 메시지로 점프(하이라이트) 또는 파일 탐색기 오픈. 정렬은 기본 **기능별 그룹**(그룹 내 최신순) / **최신순** / **오래된순**(브라우저에 기억) — 시간 정렬은 서버가 `?sort=` 로 후보를 그 방향에서 뽑아 종류별 상한이 결과를 왜곡하지 않게 한다. 각 항목의 가시성은 원래 기능의 게이트를 그대로 재사용(DM은 관리자도 멤버가 아니면 검색되지 않음).
