...
- **통합 검색:** `Ctrl/Cmd+K` 팔레트 하나로 워크스페이스 전체(개인 대화·참여 중인 방·DM/그룹 메시지·프로젝트·LLM Wiki 주제/문서·PR 리뷰·사람) 검색. 결과 클릭 시 해당 메시지로 점프(하이라이트) 또는 파일 탐색기 오픈. 정렬은 **최신순 / 오래된순** 2종(브라우저에 기억)이고, 그 아래 **기능별 필터 탭**(전체·개인 세션·대화방·DM·프로젝트·LLM Wiki·코드리뷰·사람 — 결과 있는 탭만, 건수 배지)으로 좁힌다. 탭은 히트 종류가 아니라 **기능** 단위(방 안의 메시지는 대화방, 위키 스레드 메시지는 LLM Wiki). 서버는 `?sort=`로 후보를 해당 방향에서 뽑아 종류별 상한이 결과를 왜곡하지 않게 한다. 각 항목의 가시성은 원래 기능의 게이트를 그대로 재사용(DM은 관리자도 멤버가 아니면 검색되지 않음).
