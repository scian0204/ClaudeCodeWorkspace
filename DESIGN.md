...
- **통합 검색:** `Ctrl/Cmd+K` 팔레트 하나로 워크스페이스 전체(개인 대화·참여 중인 방·DM/그룹 메시지·프로젝트·LLM Wiki 주제/문서·PR 리뷰·사람) 검색. 결과 클릭 시 해당 메시지로 점프(하이라이트) 또는 파일 탐색기 오픈. 정렬은 **최신순 / 오래된순** 2종(브라우저에 기억)이고, 그 아래 **기능별 필터 탭**(전체·개인 세션·대화방·DM·프로젝트·LLM Wiki·코드리뷰·사람 — 결과 있는 탭만, 건수 배지)으로 좁힌다. 탭은 히트 종류가 아니라 **기능** 단위(방 안의 메시지는 대화방, 위키 스레드 메시지는 LLM Wiki). 서버는 `?sort=`로 후보를 해당 방향에서 뽑아 종류별 상한이 결과를 왜곡하지 않게 한다. 가시성은 원래 기능의 게이트를 재사용하되 **개인 영역은 관리자에게도 열지 않는다**: 타인의 개인 대화(private 세션)·위키 질의 스레드·user 스코프 프로젝트·DM은 **소유자/멤버만** 검색 가능. `canViewChat`/`canAccess`는 관리자에게 단건 조회를 허용하지만, 검색은 워크스페이스 전체를 한 번에 훑는 벌크 읽기라 같은 권한으로 취급하지 않는다(단건 조회 경로는 그대로). 공유 대상(내가 속한 방, PR 리뷰 세션, LLM Wiki 지식 기반, 사용자 디렉터리)은 기존과 동일.
...
- **전자동 파이프라인(`REVIEW_AUTO`, 기본 on):** 새 PR 감지 시 서버가 (1) 로컬 머지 → (2) **무인 에이전트 턴** 1회 enqueue. 리뷰 세션 턴은 `makeAutoAllow`로 도구를 자동 승인(클래스1 경로 펜스는 유지)해 빌드/실행이 권한 프롬프트로 멈추지 않는다. 턴이 빌드·실행·버그감지·코드리뷰를 수행하고 마지막에 `VERDICT: MERGE_SAFE|DO_NOT_MERGE` + `SUMMARY:`를 출력 → `runTurn`의 `onDone` 콜백(FIFO 큐로 전달)이 파싱해 `review_sessions.verdict`에 저장. 머지 충돌이면 빌드/리뷰를 건너뛰고 verdict=conflict. **PR에 새 커밋 push 시** 폴링이 head SHA 변경을 감지해 verdict를 초기화하고 파이프라인을 재실행(재진입 가드로 동시 실행 방지).
- **리뷰 결과 PR 코멘트 게시(`REVIEW_COMMENT`, 기본 on):** 리뷰 턴이 verdict를 산출한 경우에 한해(워치독 타임아웃으로 중단된 부분 리뷰는 제외 — `setFinal`이 boolean을 반환해 실제 산출 여부를 구분) 판정 라벨 + 요약 + 리뷰 본문을 해당 PR에 코멘트로 게시(`providers.postComment`: GitHub `POST …/issues/{n}/comments`, GitLab MR notes, Bitbucket PR comments, 동일 병합권한 자격증명 사용). best-effort — 실패는 리뷰 챗 시스템 노트로만 기록하고 파이프라인은 중단하지 않음. 새 커밋 재리뷰마다 각자 코멘트를 남긴다.
- **실시간 전달:** 진행 중인 턴의 부분 blocks를 `session:join` ack로 재생하고(중간 접속자도 진행 상황 표시), 소켓 재접속(`connect`) 시 현재 세션 방에 재구독 + DB에서 메시지 재조회 → 페이지를 계속 보고 있지 않아도(재접속/중간 열람) 대화가 유실돼 보이지 않음. (메시지는 항상 서버에서 DB에 저장됨.)
- **빌드/실행 격리(샌드박스):** 리뷰 턴은 호스트 `Bash`를 차단(`disallowedTools`)하고, PR 빌드/실행/테스트를 **PR별 잠금 컨테이너**(`review/sandbox.ts`, dockerode)에서만 실행 — 워크트리만 마운트, docker 소켓 미마운트, cap drop, no-new-privileges, mem/pid 제한. 에이전트엔 인프로세스 MCP 도구 `mcp__sandbox__run`(zod, `createSdkMcpServer`)만 제공. Docker 미가용 시 호스트 실행 폴백. 잔여: egress 유지(의존성 설치). 샌드박스 이미지로 못 빌드하는 스택(.NET Framework/Windows 전용 등)은 **정적 리뷰만**(verdict에 '빌드 미실행' 명시). 컨테이너는 턴 종료/부팅 시 제거.
