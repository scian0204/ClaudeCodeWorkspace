...
[DESIGN.md](DESIGN.md) 스펙 확정(2026-07-20)부터 **v1.19.0**(2026-08-14)까지 — 커밋 **335개** 전수 기록.
...
- **fix(chat): 세션 요금제 드롭다운에 "전체 사용자"가 두 번 나옴** — 전체 사용자 함께 쓰기를 켜면 "지정 안 함(상속)" 줄과 전체 사용자 모임 줄이 똑같이 `전체 사용자` 로 찍혀서, 같은 이름이 두 개 보이고 어느 쪽이 무엇인지 알 수 없었다. 상속 줄은 이제 `자동 · <실제로 적용되는 것>` 으로 표시하고, 지금 적용 중인 항목에는 체크 표시를 넣었다. · `PENDING`
