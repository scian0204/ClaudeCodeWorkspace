...
[DESIGN.md](DESIGN.md) 스펙 확정(2026-07-20)부터 지금까지 — **v1.29.0**(2026-09-03)까지 — 커밋 **464개** 전수 기록.
...
| 버전 | 날짜 | 커밋 | 요약 |
|---|---|---|---|
| [v1.29.0](#v1290--2026-09-03) | 2026-09-03 | 2 | Claude가 쓰는 브라우저 — 페이지 열기·클릭·스크린샷, 자기 개발 서버 화면 확인 |
| [v1.28.0](#v1280--2026-09-03) | 2026-09-03 | 5 | 이미 있는 프로젝트에 세션만 가져오기, 새로고침이 필요 없는 목록, 시간을 정하는 5시간 선점 |
| [v1.27.0](#v1270--2026-09-01) | 2026-09-01 | 5 | .NET Framework 빌드를 전용 Windows 머신에서 |
...
Claude는 웹 페이지를 글로 읽을 수는 있었지만(WebFetch) *볼* 수는 없었습니다. 클릭도, 입력도, 스크린샷도 안 되고, 자기가 방금 띄운 개발 서버 화면을 확인할 방법이 없어서 UI 변경은 사람이 열어 봐야 했습니다.

이제 대화 헤더에 **브라우저** 버튼이 있습니다(입력창에서 `/browser on|off` 도 같은 일). 켜면 그 질문부터 Playwright 브라우저 도구가 붙습니다 — 페이지 열기, 클릭, 입력, 폼 채우기, 페이지를 접근성 트리(글)로 읽기, 스크린샷, 콘솔 메시지와 네트워크 요청 읽기. 스크린샷은 도구 카드 아래에 이미지로 나오고 누르면 크게 보입니다. 앱 컨테이너에서 돌든 그 대화의 빌드 컨테이너에서 돌든 자기 개발 서버에도 접속합니다 — 브라우저가 별도 컨테이너에서 돌기 때문에 거기서 `localhost` 는 브라우저 자신이라, 시스템 프롬프트가 컨테이너 이름으로 접속하고 서버는 `0.0.0.0` 에 띄우라고 알려 줍니다.

구조: Playwright MCP 컨테이너 하나(`ccw-browser`, 이미지 `browserImage`, 기본 `mcr.microsoft.com/playwright/mcp:latest`)가 내부 네트워크에서 워크스페이스 전체를 맡고, HTTP MCP 서버로 붙습니다. 질문마다 접속을 새로 열고, 서버는 접속마다 브라우저 컨텍스트(탭·쿠키·저장소)를 따로 만들어 줍니다 — 실측: 두 세션이 다른 페이지를 열어도 서로 안 보이고, 6개 동시 실행이 4초 안에 끝나며, 컨텍스트 하나에 약 40MB(유휴 80MB 위에)입니다. 그래서 컨테이너는 하나로 충분하고, 버튼은 어느 대화가 도구 정의 24개를 추가로 들고 갈지만 정합니다. 컨테이너는 처음 원하는 질문에서 켜지고, `browserIdleMs`(기본 30분) 동안 아무도 안 쓰면 정리 작업이 내립니다 — 질문이 접속을 닫지 않고 끝나 남은 컨텍스트는 서버가 스스로 정리하지 않으므로, 이 정리가 그것까지 회수합니다. 그 사이에는 `browserMemMB`(기본 2GB)가 상한이고, 다른 보조 컨테이너처럼 CapDrop ALL + no-new-privileges 입니다. 이미지에는 `--host 0.0.0.0` 과 `--allowed-hosts *` 가 필요합니다(바인드 주소가 아닌 Host 헤더에는 403을 돌려줍니다). `--shared-browser-context` 는 일부러 넣지 않습니다 — 모든 대화가 컨텍스트 하나를 공유해 한 사람의 쿠키가 다음 사람에게 넘어가기 때문입니다.

이미지가 든 도구 결과는 그동안 통째로 문자열이 되어, 스크린샷은 대화 행에 30KB짜리 base64 덩어리로 박히고 화면에는 아무것도 안 나왔습니다. 이제 이미지를 떼어 `<projects>/.attachments/<sessionId>.shots/` 에 저장하고(첨부 폴더의 형제 폴더라 첨부 개수 제한에 안 잡히고 보낼 첨부로도 안 보임) `GET /api/sessions/:id/shots/:name` 으로 첨부와 같은 읽기 권한으로 내려줍니다. 글 블록만 도구 출력이 됩니다. 고아 폴더 정리는 `.shots` 접미사를 알아봅니다.

그 밖에: `chat_sessions.browser` 컬럼, `PATCH /api/sessions/:id { browser }`(빌드 컨테이너 토글과 같은 권한), 설정 그룹 **브라우저**(`browserEnabled`, `browserImage`, `browserMemMB`, `browserIdleMs`, `browserReaperMs`)와 `/api/config` 의 `browserEnabled`, 관리자 프로세스 패널에 컨테이너 표시·중지, 부팅 시 남은 컨테이너 제거, 가이드 에이전트의 기능 설명과 "내 앱을 Claude가 보게" 레시피, README(en/ko), 정적 데모에서 브라우저 도구 3개 실행과 스크린샷 표시. PR 리뷰 질문에는 붙이지 않습니다(신뢰할 수 없는 코드를 돌리는데, 브라우저는 네트워크의 모든 컨테이너에 닿기 때문).
...
로컬 대화를 가져오려면 프로젝트 폴더를 통째로 같이 올려야 했습니다. 그 프로젝트가 이미 워크스페이스에 있어도 마찬가지라, 이미 있는 파일을 다시 올리느라 시간을 쓰고 이름이 다르면 사본이 하나 더 생겼습니다.

이제 가져오기 첫 화면에서 **이미 있는 프로젝트**(내 것 또는 공용)를 고를 수 있습니다. 고르면 곧바로 `~/.claude` 폴더 선택으로 넘어가며, 아무것도 올리지 않고 프로젝트 폴더의 파일도 건드리지 않은 채 대화내역만 들어옵니다. 폴더를 고르고 파일을 추려 새 프로젝트를 만드는 기존 방식은 그대로이고 여전히 기본값입니다.

`POST /api/import/sessions` 에 `projectId` 가 추가됐습니다. 서버는 요청한 사람이 실제로 그 프로젝트에 접근할 수 있는지(본인 것 또는 공용) 다시 확인하고 아니면 403을 돌려줍니다. 받은 id는 행을 찾는 데만 쓰이고 경로로는 절대 쓰지 않습니다.

</details>

<details>
<summary><b>feat(primer): 5시간 선점에 시간 지정</b> — 하루 종일 돌리는 대신 지정한 시각·시간대에만 · <code>ea46981</code></summary>

"5시간 창 미리 열기"는 창이 닫혀 있으면 시간을 가리지 않고 곧바로 열었습니다. 새벽 3시에 연 창은 아무도 쓰지 않는데, 밤새 다섯 시간마다 실제 메시지가 한 건씩 나갑니다.

이제 같은 스위치 아래에서 시간을 지정할 수 있습니다. **허용 시간대**(예: 09:00~19:00, 자정을 넘겨도 됩니다), **지정 시각**(예: 09:00, 14:00), 또는 둘 다(이때는 시간대 안에 드는 지정 시각만 씁니다). 비워 두면 지금까지와 똑같이 창이 닫히는 즉시 엽니다. 시각은 저장한 브라우저의 시간대 기준으로 해석하며, 그 시간대도 함께 저장됩니다.

저장 위치는 `users.prime_window_sched`(`{tz,times,from,to}`)입니다. "지금 열어도 되는가, 아니면 얼마나 기다리는가"를 판단하는 부분은 순수 함수 하나(`server/src/lib/primer-schedule.ts`)로 떼어 냈고 실행 가능한 검사가 붙어 있어, 자정을 넘기는 구간·방금 지난 시각·한쪽만 채운 구간 같은 계산을 서버 없이 확인합니다. 지정한 시간이 아닐 때는 다음 차례만 예약하고 조회조차 하지 않으므로 CLI가 아예 실행되지 않습니다. 새 설정 `windowPrimerSlotGraceMs`(기본 30분)는 지난 시각을 얼마 동안 "아직 열 차례"로 볼지 정하며, 재시작이나 실패 후 재시도로 조금 늦어져도 그 회차를 놓치지 않게 합니다. 가이드 에이전트로도 설정할 수 있습니다(`PATCH /api/auth/me` 의 `primeWindowSched`).

</details>

<details>
<summary><b>feat(realtime): 새로고침 없이 목록이 알아서 갱신</b> — 훅 하나로 모든 탭 · <code>509477f</code></summary>

바꾼 탭만 다시 읽었습니다. 동료가 만든 방, 다른 탭에서 이름을 바꾼 프로젝트, 누군가 추가한 에이전트·플러그인, 관리자가 바꾼 설정 — 나머지 탭은 사용자가 F5를 누를 때까지 옛 화면을 그대로 보여 줬습니다.

무언가를 바꾸는 40여 곳에 알림을 하나씩 붙이는 대신, 응답을 지켜보는 훅 하나를 뒀습니다. 목록에 보이는 것을 바꾸는 경로(세션·방·프로젝트·에이전트·플러그인·마켓·git 자격증명·토큰 모아쓰기·사용자·위키 주제·PR 리뷰 저장소·관리자 설정)의 요청이 성공하면 연결된 모든 탭에 신호를 보냅니다. 신호에는 아무 내용도 담기지 않고, 각 탭이 평소와 같은 조회로 자기 목록을 다시 읽으므로 원래 볼 수 없던 것이 새어 나가지 않습니다. 연달아 일어나는 변경은 300ms 단위로 한 번만 보내고, 매우 자주 일어나거나 이미 자체 실시간 갱신이 있는 하위 경로(파일 저장·git 명령·업로드·한 대화의 메시지)는 제외했습니다.

열 때 한 번만 읽던 화면(팀 에이전트·플러그인·git 자격증명·사용자 목록·대화창의 에이전트 선택)도 같은 신호로 다시 읽습니다.

</details>

<details>
<summary><b>fix(chat,plugins): UI를 처음부터 끝까지 눌러 보다 찾은 문제 3건</b> — @ 파일 참조 · git-subdir 플러그인 설치 · 인자 없는 터미널 명령 · <code>d1a7789</code></summary>

**입력창의 `@` 가 아무것도 보여 주지 않았습니다.** `@` 를 치면 대화가 보는 프로젝트의 파일·폴더 경로가 자동완성돼야 합니다. 얼마 전 앱의 모든 파일 목록이 한 폴더씩 열어 보는 방식으로 바뀌면서 조회 결과의 형식도 "파일 목록"에서 "한 폴더의 항목 목록"으로 바뀌었는데, 자동완성은 예전 형식을 그대로 읽고 있어 늘 빈 목록을 받았고 메뉴가 한 번도 뜨지 않았습니다. 이제 같은 조회에 `?flat=1` 을 주면 전체 트리를 한 번에 평평한 파일 목록으로 돌려주고(파일 5000개 상한은 그대로), 입력창은 그쪽을 씁니다.

**설치되지 않는 마켓 플러그인이 있었습니다.** 항목이 *다른 저장소의 폴더 하나*를 가리키는 경우(`claude-community` 마켓 2282개 중 401개)에 늘 "마켓 안에 플러그인 폴더가 없습니다" 로 실패했습니다. 이런 항목은 저장소 주소와 폴더 경로를 함께 갖는데, 코드가 폴더 경로를 먼저 보는 바람에 마켓 자체 사본 안에서 그 폴더를 찾았고, 거기에는 없었습니다. 이제 저장소 쪽을 먼저 판단해 설치 위치 옆에 저장소를 받아 두고, 지정된 폴더만 복사한 뒤 받아 둔 사본을 지웁니다.

**모드 없이 친 `/permissions`.** CLI가 터미널 화면으로만 그리는 명령은 워크스페이스가 대신 처리하는데, 인자가 아직 없는 명령은 그대로 CLI로 새어 나가 "/permissions isn't available in this environment." 라는 답을 받았습니다 — 이 기능이 막으려던 바로 그 답입니다. 이제는 선택지(`default|acceptEdits|bypassPermissions|plan`)를 흐리게 띄운 채 입력창에 남고, `/permissions plan` 처럼 모드를 붙이면 곧바로 바뀝니다.

가이드가 사용자에게 설명하던 문구 두 개도 사실에 맞게 고쳤습니다. 메시지 삭제는 그 메시지 하나만 지우며(뒤 내용을 지우고 다시 답하는 것은 **수정**입니다), 관리자 패널에는 사용량·비용 대시보드가 없습니다(그 화면은 없어졌습니다).
...
- **fix(sandbox): Framework SDK 이미지를 질문 도중에 받지 않는다** — 수 GB에 수십 분이 걸려서, 처음 쓸 때 받으려 하면 대화가 멈춘 것처럼 보였습니다. 이제 이미지가 없으면 "관리자 패널에서 먼저 받으세요"로 거절하고, 그 대화는 로컬 컨테이너로 돌아갑니다. 질문 사이에 컨테이너가 사라진 경우(유휴 정리, Windows 호스트 재시작)에도 도커 원문 오류가 아니라 이유를 알려 줍니다. · `7d58e77`

- **docs: Windows 빌드 호스트 설정 방법 (en/ko)** — 기능 설명에는 "관리자가 호스트 주소를 지정한다"만 있고 그 방법이 없었습니다. 이제 README 양쪽에 절차가 들어갑니다 — 어떤 Windows 에디션이 Windows 컨테이너를 돌릴 수 있는지(Home은 안 됨), 마이크로소프트 설치 스크립트, TLS로 TCP 포트를 여는 `daemon.json`, 인증서 마운트, 관리자 패널에서 할 일, 컨테이너가 안 뜰 때 확인할 것. · `bdc2ebf`
