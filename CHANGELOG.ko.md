...
<details>
<summary><b>fix(usage): 프로브 무응답 시 직전 정상 한도 표시</b> — 부하 중에도 팝오버가 비지 않게 · <code>d0ff87d</code></summary>

부하가 높을 때(예: 팀 에이전트 턴이 CLI 서브프로세스 여럿 스폰) 사용량 프로브 자신의 CLI 콜드 스타트가 `usageProbeTimeoutMs`를 넘겨 굶는 경우가 있는데, 그러면 몇 분 전까지 한도를 잘 보여주던 로그인 계정인데도 팝오버가 "조회 불가"로 떨어졌다. 이제 **계정 단위 last-known-good** 한도를 보관해, 조회가 응답을 아예 못 받으면(타임아웃·오류) 그것을 대신 내려준다 — API 키처럼 "한도 없음"이라고 실제로 **응답한** 경우는 진짜 답이므로 가리지 않는다. 새 설정: `usageLastGoodTtlMs`(기본 30분, 0이면 비활성). 무응답 프로브는 `[usage]` 경고 한 줄을 남겨 다음 제보 때 진단 가능하게 했다.
...
`probeUsage`가 결과를 무조건 캐시했다 — 계정 한도 조회가 `null`(타임아웃/에러)로 끝난 프로브까지. 호스트가 바쁠 때(예: 같은 머신에서 이미지 빌드 중)는 45초 조회 창을 넘기기 쉽고, 그 실패가 `usageProbeTtlMs` 내내 고정돼 팝오버를 다시 열어도 재시도 대신 실패를 재제공했다. 이제 응답이 온 조회만 캐시하고, null 조회는 캐시 엔트리를 지워 다음 열기에 재시도한다. 팝오버(원래 열 때마다 재조회)에 서버 캐시를 완전히 우회하는 `?fresh=1` 새로 조회 버튼 추가. 기반 조회 자체는 라이브 컨테이너 안에서 검증(일반 로그인 경로·자격증명 저장소 재지정 경로 모두 실제 한도 반환).
...
브라우저 로그인을 했는데도 팝오버가 "user:profile 스코프 없음"이라고 했다: 붙여넣은 토큰이 턴 실행용 인증 해석에서 의도적으로 우선되는데, 플랜 한도 조회까지 그 토큰으로 돌았고 `claude setup-token`은 추론 전용이라 항상 실패. 이제 계정 단위 한도 **조회**만 요청자의 전체 스코프 브라우저 로그인을 우선 사용(`CLAUDE_SECURESTORAGE_CONFIG_DIR`로 자격증명 저장소만 재지정 — 턴은 기존 우선순위 유지), 로그인이 없으면 "마이페이지에서 로그인"이라는 실행 가능한 안내를 표시. 진짜 플랜 한도가 뜨게 되면서 자체 집계 "실사용량 (워크스페이스 집계)" 섹션은 요청대로 전부 제거 — 팝오버 UI·엔드포인트 `spend` 필드·`spendSummary`+테스트·데모 시드·`usage.spend*` i18n 키. `recordUsage` 기록은 유지(행 비용 미미, 리소스 정리가 관리).
...
<details>
<summary><b>feat(agents): 팀/개인 커스텀 에이전트를 모든 세션에 적용</b> — Task 툴 서브에이전트 + 메인 스레드 페르소나 피커 · <code>fce75e7</code></summary>
