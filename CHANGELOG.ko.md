...
[DESIGN.md](DESIGN.md) 스펙 확정(2026-07-20)부터 **v1.21.0**(2026-08-19)까지 — 커밋 **390개** 전수 기록.
...
- **`/hooks` 를 다시 연결 해제** — v1.21.0에서 관리자 설정으로 보냈는데 잘못된 연결이었다. 워크스페이스에 hooks 편집 화면이 없어서, 요청한 일을 할 수 없는 페이지로 데려다 놓는 셈이었다. 이제 대응 화면이 없는 다른 명령(`/bug`·`/install-github-app`)처럼 CLI의 원래 "여기서는 안 됨" 답을 그대로 돌려준다. `/privacy-settings` 는 계속 관리자 설정을 연다.
