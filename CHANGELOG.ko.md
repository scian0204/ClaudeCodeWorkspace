...
[DESIGN.md](DESIGN.md) 스펙 확정(2026-07-20)부터 **v1.17.2**(2026-08-14)까지 — 커밋 **322개** 전수 기록.
...
- **fix(sessions): 대화방에 모임·빌드 컨테이너를 지정하려면 그 방 참여자여야 함** — `PATCH /api/sessions/:id` 는 대화방의 공용 행을 `canEditChat` 으로 막는데, 이 함수는 로그인한 사용자면 모두 통과시킨다(대화 세션 ID를 못 맞힌다는 전제). 모델 드롭다운에는 그래도 되지만 v1.17.0에서 추가한 두 필드는 대가가 따른다 — 하나는 그 방의 질문이 *누구의* Claude 요금제를 쓸지 정하고, 다른 하나는 컨테이너를 띄운다. 이제 `poolId` 와 `sandbox` 는 그 방에 질문을 보낼 수 있는 권한(관리자 또는 참여자)을 요구한다. 같은 요청의 나머지 필드는 그대로다. · `PENDING`
